...
$totalclosed = $mysqli->query("SELECT COUNT(`status_id`) AS number FROM `ost_ticket` WHERE `status_id` = 2")->fetch_object()->number;
$totalresolved = $mysqli->query("SELECT COUNT(`status_id`) AS number FROM `ost_ticket` WHERE `status_id` = 3")->fetch_object()->number;
